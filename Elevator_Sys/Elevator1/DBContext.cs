...
namespace Elevator1
{
    internal class DBContext
    {
        ...
        string connecting = @"Server = LAPTOP-O2PGQJ7M\MYFIRSTSQL ; Database = TimerForm; Trusted_Connection = True";
        ...
        public void loadLogsFormDB(DataTable dt, DataGridView dataGridViewLogs)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connecting))
                {
                    string query = @"Select LogTime, EventDescription from Logs order by LogTime";
                    using (SqlDataAdapter adapter = new SqlDataAdapter(query, conn))
                    {
                        dt.Rows.Clear();
                        adapter.Fill(dt);
